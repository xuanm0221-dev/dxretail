...
export default function MLBKIDSDashboard() {
  ...
  const fetchData = async () => {
    try {
      setLoading(true);
      const response = await fetch(`/api/sales-report?brand=I&year=${selectedYear}`);
  ...
  const shopRows = useMemo(() => {
    ...
    const monthCount = (selectedYear === '2025') ? 11 : 12; // 2025년 MLB KIDS는 11월까지
  ...
  const analysisCards = useMemo(() => {
    if (summaryRows.length === 0 || shopRows.length === 0) {
      return [
        { id: "trend", label: "월별 추세", description: "데이터 로딩 중...", color: "green" },
      ...
      <div className="max-w-full mx-auto">
        ...
        <div className="sticky top-0 z-50 bg-gray-100 rounded-xl mb-6 px-6 py-5">
          <div className="flex items-center justify-between mb-4">
            <div className="text-center flex-1">
              <h1 className="text-5xl font-bold text-green-600 tracking-wide">
                MLB KIDS 매장별 리테일 매출
              </h1>
            </div>
          </div>
          <div className="flex justify-center">
            <BrandTabs currentBrand="mlb-kids" />
          </div>
        </div>
            ...
            <div className="grid gap-4 grid-cols-1 lg:grid-cols-4">
              ...
              <div className="lg:col-span-3 grid gap-4 md:grid-cols-3">
                {analysisCards.map((card) => {
                  ...
                  const style = colorStyles[card.color] || colorStyles.green;
        ...
        <div className="mt-6 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 bg-white/60 backdrop-blur-sm rounded-xl p-4 shadow-md">
          <p className="text-sm text-gray-600 flex items-center gap-2">
            ...
            <span>
              {selectedYear === '2025' 
                ? 'MLB KIDS 브랜드는 2025년 1~11월 데이터만 표시됩니다.' 
                : `MLB KIDS 브랜드 ${selectedYear}년 데이터를 표시하고 있습니다. (1~12월)`
              }
            </span>
          </p>
          <button
